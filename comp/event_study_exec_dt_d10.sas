...
run;
%print30(for_event_study_exec_dt_temp)
%contents(for_event_study_exec_dt_temp)


data exec_dt_study_foreign_d10; *4285;
      format permno exec_dt;
      set for_event_study_exec_dt_temp( where=(foreign_tran=1 AND deciles_for=10)
                                        keep=permno exec_dt foreign_tran deciles_for);
                                        drop foreign_tran deciles_for;
run;
%print30(exec_dt_study_foreign_d10)
...
aa\event_Study\exec_dt_study_relation.txt" 
     ...
     PUTNAMES=YES;
RUN;
PROC EXPORT DATA= WORK.exec_dt_study_foreign_d10 
            OUTFILE= "C:\Users\lihon\OneDrive - Kent State University\aaaa\event_Study\exec_dt_study_foreign_d10.txt" 
            DBMS=TAB REPLACE;
     PUTNAMES=YES;
RUN;
